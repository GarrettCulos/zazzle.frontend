...
import { RenderView } from '../atomic/wysiwyg/read-only';
...
interface ProjectDescriptionInterface {
  description: string;
}

const ProjectDescription: React.FC<ProjectDescriptionInterface> = ({ description }: ProjectDescriptionInterface) => {
  try {
    const slateDescription = JSON.parse(description);
    return <RenderView nodes={slateDescription} />;
  } catch (err) {
    return <div></div>;
  }
};
  ...
  return (
    <ProjectTileContainer>
      <ProjectFlex column>
        ...
        <Description>{project.description && <ProjectDescription description={project.description} />}</Description>
